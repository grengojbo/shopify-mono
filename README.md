...
1. **Створити custom app**: [Partner Dashboard](https://partners.shopify.com) → Apps → Create app → Create app manually.

   ⚠️ **Важливо: апп і магазин мають належати одній Partner/Dev-організації.** Shopify не дозволяє встановити custom-апп на магазин з іншої організації (навіть якщо на обох акаунтах один email) — розширення просто не з'явиться в списку блоків Checkout-редактора, без явної помилки. Перевір, якій організації належить твій магазин, **перед** створенням апа:
   - У Dev Dashboard (`dev.shopify.com/dashboard`) назва організації видна у верхньому правому кутку.
   - У Partner Dashboard можна перейти напряму за ID організації: `https://partners.shopify.com/<org_id>/apps` (404 на цьому лінку означає, що організація існує лише в новому Dev Dashboard і не має класичного Partner Dashboard — це нормально, просто продовжуй роботу в `dev.shopify.com/dashboard`).
   - Якщо помилився з організацією вже після лінкування — виправляється через `npm run link -- --reset` (Крок 2 нижче), обравши правильну організацію в інтерактивному промпті. Це створить **новий** апп із новим `client_id`/`client_secret` — стару версію в неправильній організації можна просто лишити невикористаною або видалити.
...
2. Скопіюй **App Automation Token** (Токен автоматизации приложения) у кабінеті розробника: твій додаток у Dev Dashboard → розділ **Settings** (Настройки) в лівому меню → прокрути в самий низ до секції **App automation token**. Тобі потрібен токен, який починається на `atkn_` (наприклад, `atkn_e1b...`, він зазвичай має обмежений термін дії, що є абсолютно нормальним для токенів автоматизації).
...
⚠️ **Токен прив'язаний до конкретного апа.** Якщо колись перелінкуєш `app/` на інший апп (інший `client_id`, наприклад через `npm run link -- --reset` — див. попередній розділ про організації), старий `SHOPIFY_APP_AUTOMATION_TOKEN` перестане працювати — треба згенерувати новий у **новому** апі й оновити секрет тим самим `gh secret set`.
---
