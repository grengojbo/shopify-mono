...
- **monobank acquiring reference** — this repo vendors Monobank's official AI-tools skill at [`.claude/skills/monobank-acquiring/`](.claude/skills/monobank-acquiring/) (source: https://monobank.ua/api-docs/acquiring/dev/ai-tools/docs--ai-skills), committed so every contributor and every Claude Code session gets it, not just one machine. It is the primary source for endpoint schemas, error codes, and webhook details — start from `SKILL.md` (index, common types, error codes, invoice statuses), then the per-section files: `invoice.md`, `webhook.md`, `payment.md`, `qr.md`, `wallet.md`, `merchant.md`, `statement.md`, `fiscal.md`, `quickstart.md`. `examples/<lang>/` has runnable single-file reference servers (Python/Node/Go/PHP/C#/Java) covering invoice creation, status polling, and webhook verification — useful for confirming behavior, but **do not port their signature-verification code as-is**: the Node.js example uses `crypto.createVerify` (Node's ECDSA verify accepts DER natively), which sidesteps the DER→P1363 problem entirely. This repo's Tech stack mandates `crypto.subtle` in Workers, where the DER→P1363 converter described in Architecture stays mandatory regardless of what the reference examples do. Fall back to the live docs (`api.monobank.ua/docs/acquiring.html`) only if the vendored skill is stale or silent on a field — and if it's out of date, re-sync it from the source URL above rather than patching around it. If you don't see `monobank-acquiring` in your available-skills list, it was added mid-session; it'll be picked up from the next session onward.
